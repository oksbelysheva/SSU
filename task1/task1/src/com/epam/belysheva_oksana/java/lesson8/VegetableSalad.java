...
import com.epam.belysheva_oksana.java.lesson8.Exception.VegetableIsNotExistException;
...
public class VegetableSalad implements Serializable {
    ...
    public double getSumCalories() {
        ...
        for (Vegetable vegetable : compositionOfTheSalad) {
            sum += (vegetable.getNutritiveValue().getCalories() * vegetable.getWeight());
        }
    ...
    public double getSumProteins() {
        ...
        for (Vegetable vegetable : compositionOfTheSalad) {
            sum += (vegetable.getNutritiveValue().getProtein() * vegetable.getWeight());
        }
    ...
    public double getSumCarbohydrates() {
        ...
        for (Vegetable vegetable : compositionOfTheSalad) {
            sum += (vegetable.getNutritiveValue().getCarbohydrates() * vegetable.getWeight());
        }
    ...
    public Set<Vegetable> getVegetableByCalories(double lowerBound, double upperBound) {
        ...
        for (Vegetable vegetable : compositionOfTheSalad) {
            if (vegetable.getNutritiveValue().getCalories() >= lowerBound && vegetable.getNutritiveValue().getCalories() <= upperBound) {
